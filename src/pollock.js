...
//var size = 1000;
var dpr = window.devicePixelRatio * 0.64;
        ...
        if (e.clientX < 1395) {
            if (e.clientY > 0) {
                if (e.clientY < 750 - radius) {
                    ...
                    c.fill();
                    c.arc(1650 - e.clientX, 750 - e.clientY, radius, 0, Math.PI * 2);
                    c.fill();
...
var dribble = function (e) {
    ...
    if (dribble_count > 1) {
         ...
         c.arc(1650 - e.clientX + ranX, 720 - e.clientY + ranY, ranR2, 0, Math.PI * 2);
         c.fill();
         dribble_count = 0;
    }
    c.fillStyle = temp;
...
function submitForm() {
     ...
     c.strokeStyle = 'Black';
     c.fillStyle = 'Black';
     c.fillText(val, canvas.width-250, 550);
